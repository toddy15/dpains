...
<x-input name="year" id="year" value="{{ old('year', $due_shift->year ?? '') }}"
...
<x-input name="nights" id="nights" type="number" value="{{ old('nights', $due_shift->nights ?? '') }}"
    invalid="{{ $errors->has('nights') }}" />

<x-label for="nefs" value="NEF-Schichten:" />
<x-input name="nefs" id="nefs" type="number" value="{{ old('nefs', $due_shift->nefs ?? '') }}"
